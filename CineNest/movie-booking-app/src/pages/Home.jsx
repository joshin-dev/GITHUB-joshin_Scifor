...
function Home() {
  ...
  useEffect(() => {
    const fetchSearchResults = async () => {
      ...
      try {
        const res = await axios.get(
          `https://api.themoviedb.org/3/search/movie?api_key=851b739379fb869940a149644577eff6&query=${encodeURIComponent(searchTerm)}`
        );
        // Filter search results to show only movies that are currently playing
        const searchMovies = res.data.results.filter(movie => movie.poster_path);
        
        // Cross-reference with now playing movies for more accurate results
        const nowPlayingIds = nowPlayingMovies.map(movie => movie.id);
        const currentlyPlayingSearchResults = searchMovies.filter(movie => 
          nowPlayingIds.includes(movie.id)
        );
        
        // If no currently playing movies found, show all search results
        setSearchResults(
          currentlyPlayingSearchResults.length > 0 
            ? currentlyPlayingSearchResults.slice(0, 12)
            : searchMovies.slice(0, 12)
        );
      } catch (error) {
        ...
        {searchTerm ? (
          <>
            ...
            {searchResults.length ? (
              ...
              !searching && <p style={{ position: 'relative', zIndex: 10 }}>No movies found for "{searchTerm}" in current theaters</p>
            )}
          </>
        ) : (
          <>
